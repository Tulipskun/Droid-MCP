...
The current executor expects an adb executable available to the APK process. The MCP transport and tool layer are isolated from the execution backend so the bridge can be replaced without changing the MCP interface.
